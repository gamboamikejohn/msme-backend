...
exports.resendVerification = exports.verifyEmail = exports.refresh = exports.login = exports.register = void 0;
...
const generateTokens = (userId) => {
    const accessToken = jsonwebtoken_1.default.sign({ userId }, JWT_SECRET, { expiresIn: process.env.JWT_EXPIRES_IN || '15m' });
    const refreshToken = jsonwebtoken_1.default.sign({ userId }, JWT_REFRESH_SECRET, { expiresIn: process.env.JWT_REFRESH_EXPIRES_IN || '7d' });
...
const register = async (req, res) => {
    try {
        ...
        if (role === 'MENTEE') {
            try {
                let generalGroup = await database_1.default.chatGroup.findFirst({
                    where: { isGeneral: true }
                });
                if (!generalGroup) {
                    generalGroup = await database_1.default.chatGroup.create({
                        data: {
                            name: 'General Chat',
                ...
                await database_1.default.groupMember.create({
                    data: {
...
const resendVerification = async (req, res) => {
    try {
        ...
        try {
            await (0, email_1.sendVerificationEmail)(email, user.name, verificationToken);
            res.json({
                success: true,
